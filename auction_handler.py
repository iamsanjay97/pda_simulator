...
iters = list(range(1, config.iters))
 
with multiprocessing.Pool() as pool: 
    results = pool.map(auction, iters) 

for buyer in results[0].keys():
    print('\nBuyer:', buyer)
    print('Average Purchase Cost', mean([result[buyer] for result in results]))
    print('SD of Purchase Cost', stdev([result[buyer] for result in results]))

end = time.time()

print('\n#Iterations: {}, Total time taken: {}'.format(config.iters, (end-start)))
